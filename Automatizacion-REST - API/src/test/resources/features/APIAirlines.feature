...
Feature: API Aerolineas
  ...
  @CP03
  Scenario Outline: Actualizar una aerolínea exitosamente
    Given el actor establece el endpoint PUT para actualizar una aerolínea
    When el actor envía una solicitud HTTP PUT con el "<_id>" "<name>" "<country>" "<logo>" "<slogan>" "<head_quaters>" "<website>" "<established>"
  ...
  @CP04
  Scenario Outline: Eliminar una aerolínea exitosamente
    Given el actor establece el endpoint DELETE para eliminar una aerolínea
    When el actor envía una solicitud HTTP DELETE con el "<_id>"
